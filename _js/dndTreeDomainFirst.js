...
d3.select("svg")
       .remove();
d3.select("#gradeDomainSwitch").remove();
d3.select("#subjectFilter").remove();
d3.select("#gradeFilter").remove();
d3.select("#domainFilter").remove();
d3.select("#clusterFilter").remove();
...
treeJSON = d3.text("./_data/flareDomainFirst.txt", function(error, treeData) {
  ...
  /*      node.select('text')
    ...
    // Building the grade domain switch (http://stackoverflow.com/questions/19302318/radio-buttons-in-d3-how-to-align-text-correctly-and-select-a-default)
    d3.select(".gradeDomainSwitch")
        .append("form").attr("id", "gradeDomainSwitch")
        .selectAll("label")
        .data(gradeDomainSwitchOptions)
        .enter()
        .append("label").text(function(d) {return gradeDomainSwitchLabels[d];})
        .insert("input")
            .attr({
                type: "radio",
                name: "gradeDomainSwitch",
                value: function(d){ return gradeDomainSwitchValues[d];}
            }) 
            .property("checked", function(d, i) {return i===gradeDomainDefaultSwitch;})
